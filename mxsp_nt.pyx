...
Attributes:
    ...
    READ_ONLY (set): 只读socket对象集合
    READ_WRITE (set): 读写socket对象集合
"""
...
cpdef setRead(object sock):
    ...
    if sock is not None:
        READ_ONLY.add(sock)
        READ_WRITE.discard(sock)


cpdef setWrite(object sock):
    """设置socket可写
...
cpdef setNone(object sock):
    ...
    """
    global READ_ONLY, READ_WRITE
    READ_ONLY.discard(sock)
    READ_WRITE.discard(sock)


cpdef writeLog(str logmsg, int loglevel):
    """写日志
...
cdef class SocketSession:
    ...
    cdef public int connect_fail_count, fileno, connected, debug, idx, reconnect_timer
    cdef public double connect_time, last_send_time, lost_connect_time
    cdef public object sock
    cdef public str wait_send_data
    def __init__(self, int idx, tuple address, int debug=0):
        """初始化
    ...
    cpdef _showDebug(self, str msg, int loglevel=20):
        """
        显示调试信息
    ...
    cpdef setSendData(self, str sendData):
        """设置发送缓存
...
cdef class SocketPool:
    ...
    """
    cdef public tuple address
    cdef public int debug, max_open, max_idle, max_recycle_sec
    cdef public object send_queue, free_clients
    cdef public set working_clients
    def __init__(self,
                 tuple connect_address,
                 int max_idle_connections=1,
                 int max_open_connections=20,
                 int max_recycle_sec=60,
                 int debug=0,
                 object logger=None):
        """初始化
    ...
    cpdef int expandPool(self, int new_open_connections):
        """扩大连接池数量
    ...
    cpdef addSendMsg(self, str msg, int pri=5):
        """向发送队列添加数据
    ...
    cpdef object getClient(self, object fn):
    ...
    cpdef start(self):
        ...
        cdef double last_gc = _time.time()
        cdef double t
        cdef int reconnect_sec = 15
        while True:
            _time.sleep(0.01)
            ...
            for c in x:
                # 断线重连
                if not c.isNothingSend() and not c.isConnected() and t - c.lost_connect_time > reconnect_sec:
                    c.connect()
                    continue
            ...
            if len(READ_ONLY) + len(READ_WRITE) == 0:
                _time.sleep(0.1)
            else:
                # inbuf, outbuf, errbuf = _select.select(READ_ONLY, READ_WRITE, READ_ONLY, 1)
                try:
                    # 获取事件
                    inbuf, outbuf, errbuf = _select.select(READ_ONLY, READ_WRITE, READ_ONLY, 1)
                except Exception as ex:
    ...
    cpdef mainLoop(self, object sock, str eve, int debug=0):
        """
        主循环
    ...
    cpdef worker(self, object sock, str eve):
        """
        事件分类处理
